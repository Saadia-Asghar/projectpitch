...
function Slide06Demo({ active }: SlideProps) {
  return (
    <div className="slide" style={{
      ...
      <div style={{ flex: 1 }}>
        <div className="anim-1" style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.2em", color: "#4CD98B", textTransform: "uppercase", marginBottom: 16 }}>Product Demo</div>
        ...
        <div className="anim-3" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {[
            { step: "01", label: "Enter amount + currency" },
            { step: "02", label: "Compare all channels, ranked by PKR received" },
            { step: "03", label: "Set alert for target rate" },
            { step: "04", label: "Log transfer when sent, track status" },
          ].map(({ step, label }) => (
            <div key={step} style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{
                ...
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 12, fontWeight: 800, color: "#1A9E5A", fontFamily: "'DM Mono', monospace",
              }}>{step}</div>
              <div style={{ fontSize: 16, color: "rgba(255,255,255,0.75)", fontWeight: 500 }}>{label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="anim-4 float" style={{ flexShrink: 0 }}>
        <div style={{
          width: 240,
          height: 480,
          background: "linear-gradient(160deg, #0D2045, #0A1628)",
          borderRadius: 40,
          border: "2px solid rgba(255,255,255,0.12)",
          boxShadow: "0 40px 80px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,255,255,0.05) inset, 0 0 60px rgba(26,158,90,0.15)",
          overflow: "hidden",
          position: "relative",
        }}>
          {/* Status bar */}
          <div style={{ height: 36, background: "rgba(0,0,0,0.3)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px" }}>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", fontFamily: "'DM Mono', monospace" }}>9:41</div>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)" }}>●●●</div>
          </div>
          {/* Header */}
          <div style={{ padding: "12px 16px 8px", background: "rgba(0,0,0,0.2)" }}>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)" }}>Sending</div>
            <div style={{ fontSize: 28, fontWeight: 900, color: "#fff", fontFamily: "'DM Mono', monospace" }}>$1,000</div>
            <div style={{ fontSize: 11, color: "rgba(26,158,90,0.8)", marginTop: 2 }}>Live rate · Updated just now</div>
          </div>
          {/* Channels */}
          {[
            { name: "TCS Sentral", pkr: "279,200", fee: "$0", tag: "BEST", color: "#1A9E5A" },
            { name: "Wise", pkr: "277,300", fee: "$4.99", tag: "", color: "" },
            { name: "Western Union", pkr: "269,500", fee: "$9.99", tag: "WORST", color: "#C0392B" },
          ].map(({ name, pkr, fee, tag, color }) => (
            <div key={name} style={{
              margin: "6px 12px",
              background: "rgba(255,255,255,0.05)",
              borderRadius: 12,
              padding: "10px 12px",
              border: tag === "BEST" ? "1px solid rgba(26,158,90,0.4)" : "1px solid transparent",
              display: "flex",
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  {tag && <div style={{ fontSize: 9, fontWeight: 800, color, background: `${color}22`, borderRadius: 4, padding: "1px 5px" }}>{tag}</div>}
                </div>
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)" }}>Fee {fee}</div>
              </div>
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 13, fontWeight: 700, color: tag === "BEST" ? "#4CD98B" : "#fff" }}>₨{pkr}</div>
            </div>
          ))}
          {/* Bottom nav */}
          <div style={{
